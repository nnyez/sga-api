/**
 * Generador de PDF para el tipo "Informe de Seguimiento".
 *
 * Incluye secciones parametricas (SB: 11 campos select) y de observacion
 * ademas de las secciones base (informacion general, antecedentes, firma).
 *
 * La funcion renderSections (de utils.ts) recorre las secciones de la plantilla
 * y genera:
 * - Tabla de parametros SB con una columna por docente/asignatura
 * - Bloques de observaciones con acordeon por docente
 * - Secciones de texto libre (objetivos, analisis)
 */
...
export function generate(data: ReportData): TDocumentDefinitions {
  const items: ReportItem[] = [
    { type: 'header_title', text: data.title || 'Informe de Seguimiento' },
    {
      type: 'info_table',
      rows: [
