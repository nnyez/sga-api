/**
 * Generador de PDF para el tipo "Informe de Seguimiento con Visita Aulica".
 *
 * Similar al informe de seguimiento pero con parametros VA (6 campos select)
 * y 5 bloques de observacion que incluyen:
 * - Observaciones de los estudiantes al docente
 * - Analisis de calificaciones
 * - Acciones de mejora sugeridas
 * - Observaciones del Docente a la Materia
 * - Acciones de Mejora del Docente a la materia
 *
...
export function generate(data: ReportData): TDocumentDefinitions {
  const items: ReportItem[] = [
    { type: 'header_title', text: data.title || 'Informe de Seguimiento y Visita Áulica' },
    {
      type: 'info_table',
      rows: [
