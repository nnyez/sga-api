/**
 * Generador de PDF para el tipo "Informe Inicial".
 *
 * Construye la lista de ReportItems en el orden del documento:
 * 1. Titulo principal
 * 2. Tabla de informacion general (carrera, periodo, area, fecha, jefe)
 * 3. Tabla de asignaturas/docentes
 * 4. Secciones dinamicas (renderSections): Antecedentes, Actividades, Analisis
 * 5. Firma
 *
...
export function generate(data: ReportData): TDocumentDefinitions {
  const items: ReportItem[] = [
    { type: 'header_title', text: data.title || 'Informe Inicial' },
    {
      type: 'info_table',
      rows: [
